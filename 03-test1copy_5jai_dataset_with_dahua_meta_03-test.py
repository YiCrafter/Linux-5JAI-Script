...
在原有功能基础上，新增复制：
1. dahua_window.png
2. _dahua_paste_back_meta.json

这样后续 pix2pixHD 推理得到预测结果后，可以根据同一个 sample_id 找到
对应的大华车窗图和贴回元信息，方便执行贴回操作。
...
│   └── 20260602_000001.png
├── paste_back_meta/
│   └── 20260602_000001.json
└── manifests/
    ├── 20260602_manifest.csv
    └── 20260602_skipped.csv

命名逻辑：
同一组数据统一使用同一个 sample_id，例如：
GT/20260602_000001.npy
RGB/20260602_000001.npy
DAHUA_Window/20260602_000001.png
paste_back_meta/20260602_000001.json

后续推理结果如果命名为 20260602_000001.png，
即可直接找到对应的：
DAHUA_Window/20260602_000001.png
paste_back_meta/20260602_000001.json
"""
...
DAY_DATA_DIR = Path(r"/mnt/bigdata/ndy-5JAI/01_processed_data/processed_2026-05-27/_test")

# 每日日期前缀，用于生成统一文件名
# 例如 DATE_PREFIX = "20260602"，输出文件名为 20260602_000001.npy
DATE_PREFIX = "20260527"

# 整理后的训练数据保存路径
DEST_ROOT = Path(r"/mnt/bigdata/ndy-5JAI/dataset-5JAI/test2--train")
...
# 是否自动跳过已经存在的编号
# True：如果 20260602_000001.npy 已存在，会自动尝试 20260602_000002.npy
...
def copy_group_to_dataset(group_dir: Path, sample_id: str, info: dict):
    ...
    同时复制 dahua_window.png 和 _dahua_paste_back_meta.json。
    """
